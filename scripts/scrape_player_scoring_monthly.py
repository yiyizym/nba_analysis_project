#!/usr/bin/env python3
"""
Scrape NBA Player Scoring Stats (including Unassisted FG%) for monthly periods.

This script scrapes player-level scoring statistics with monthly granularity.
Key columns: FGM %AST, FGM %UAST (Unassisted FG%), 2FGM %UAST, 3FGM %UAST

URL pattern: https://www.nba.com/stats/players/scoring?SeasonType=Regular+Season&Season={season}&Month={month}
"""
...
PROGRESS_FILE = Path("data/newly_scraped/player_monthly/player_scoring_progress.json")
...
def get_task_key(season, month):
    return f"{season}_scoring_{month}"
...
def construct_player_url(season, month_value, season_type="Regular+Season"):
    """Construct URL for NBA player scoring stats page."""
    base_url = "https://www.nba.com/stats/players/scoring"
...
def main():
    print("=" * 70)
    print("Player Scoring Stats Scraper (FGM %UAST, Monthly)")
    print("=" * 70)
    ...
    try:
        ...
        app_logger.setup("scrape_player_scoring.log")
        ...
        for season, season_info in SEASONS.items():
            ...
            for month_name, month_value in season_info["months"].items():
                ...
                try:
                    url = construct_player_url(season, month_value)
                    ...
                    if df is not None and not df.empty:
                        df["Month"] = month_name
                        df["Season"] = season
                        df["StatCategory"] = "scoring"

                        filename = f"player_scoring_{month_name}.csv"
