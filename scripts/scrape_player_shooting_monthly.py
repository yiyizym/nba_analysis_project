#!/usr/bin/env python3
"""
Scrape NBA Player Shooting stats for monthly periods.

This script scrapes player shooting statistics with monthly granularity.

URL pattern: https://www.nba.com/stats/players/shooting?SeasonType=Regular+Season&Season={season}&Month={month}&DistanceRange={range}

Distance Ranges:
- By+Zone: Restricted Area, Paint (Non-RA), Mid-Range, Left/Right Corner 3, Above Break 3
- 5ft+Range: Less Than 5 ft, 5-9 ft, 10-14 ft, 15-19 ft, 20-24 ft, 25-29 ft
"""
...
# Shooting categories with extra params
SHOOTING_CATEGORIES = {
    "shooting_by_zone": {
        "category": "shooting",
        "extra_params": {"DistanceRange": "By+Zone"},
        "file_prefix": "player_shooting_by_zone"
    },
    "shooting_5ft_range": {
        "category": "shooting",
        "extra_params": {"DistanceRange": "5ft+Range"},
        "file_prefix": "player_shooting_5ft_range"
    },
}
...
PROGRESS_FILE = Path("data/newly_scraped/player_monthly/player_shooting_progress.json")
...
def get_task_key(season, category_key, month):
    return f"{season}_{category_key}_{month}"
...
def construct_shooting_url(category, season, month_value, extra_params, season_type="Regular+Season"):
    """Construct URL for NBA player shooting page."""
...
def scrape_shooting_stats(page_scraper, config, url):
    """Scrape player shooting from a URL and return DataFrame."""
    try:
...
def scrape_with_retry(page_scraper, config, url, max_retries=MAX_RETRIES):
    ...
    for attempt in range(max_retries + 1):
        try:
            df = scrape_shooting_stats(page_scraper, config, url)
...
def main():
    print("=" * 70)
    print("Player Shooting Scraper (By Zone + 5ft Range, Monthly)")
    print("=" * 70)
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Categories: {list(SHOOTING_CATEGORIES.keys())}")
    print()
    ...
    total_tasks = sum(
        len(SHOOTING_CATEGORIES) * len(season_info["months"])
    ...
    try:
        ...
        app_logger.setup("scrape_player_shooting.log")
        ...
        for season, season_info in SEASONS.items():
            ...
            for category_key, category_info in SHOOTING_CATEGORIES.items():
                print(f"\n  {category_key}:")

                for month_name, month_value in season_info["months"].items():
                    task_key = get_task_key(season, category_key, month_name)
                    ...
                    try:
                        url = construct_shooting_url(
                            category_info["category"],
                            season,
                            month_value,
                            category_info["extra_params"]
                        )
                        ...
                        if df is not None and not df.empty:
                            df["Month"] = month_name
                            df["Season"] = season
                            df["StatCategory"] = category_key

                            filename = f"{category_info['file_prefix']}_{month_name}.csv"
